# harnest Coordination Protocol

You are running in **harnest tmux mode** — a split-pane tmux session where each agent is a separate `claude` process. You coordinate with other agents through the `.harnest/` filesystem directory, NOT through built-in `TeamCreate`, `SendMessage`, `TaskCreate`, `TaskUpdate`, `TaskList`, or `TaskGet` tools.
...
```bash
cat .harnest/config.json
```

Returns: `{"session_name": "harnest", "started_at": "<ISO timestamp>", "task": "<user's task description>", "agents": [...]}`

## Task Management

### List tasks
```bash
ls .harnest/tasks/task-*.json 2>/dev/null
for f in .harnest/tasks/task-*.json; do cat "$f"; echo; done
```

### Read a task
```bash
cat .harnest/tasks/task-001.json
```
...
```bash
TASK_ID=$(ls .harnest/tasks/task-*.json 2>/dev/null | wc -l)
TASK_ID=$((TASK_ID + 1))
TASK_FILE=$(printf ".harnest/tasks/task-%03d.json" "$TASK_ID")
TMP=$(mktemp)
cat > "$TMP" << 'TASKEOF'
{
  "id": TASK_ID_HERE,
...
python3 -c "
import json
with open('.harnest/tasks/task-001.json') as f:
    t = json.load(f)
t['status'] = 'in_progress'
t['owner'] = 'jr-engineer-1'
with open('$TMP', 'w') as f:
    json.dump(t, f, indent=2)
" && mv "$TMP" .harnest/tasks/task-001.json
```
...
MSGEOF
mv "$TMP" ".harnest/messages/${TS}-${FROM}-${TO}.json"
```

### Check for messages
```bash
# List messages addressed to you
ls .harnest/messages/*-*-your-agent-name.json 2>/dev/null
# Read latest message
ls -t .harnest/messages/*-*-your-agent-name.json 2>/dev/null | head -1 | xargs cat
```
...
STATUSEOF
mv "$TMP" .harnest/status/your-agent-name.json
```
...
```bash
echo '{"ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","agent":"your-agent-name","action":"task_started","message":"Starting task 3: Implement login form"}' >> .harnest/log.jsonl
```
...
ls -t .harnest/messages/*-*-your-agent-name.json 2>/dev/null | head -5
ls .harnest/tasks/task-*.json 2>/dev/null | while read f; do
...
4. **Update your status** — keep `.harnest/status/<your-name>.json` current.
5. **Log your activity** — append to `.harnest/log.jsonl` for monitor visibility.
